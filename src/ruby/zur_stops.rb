...
class ZurStops 
  ...
  def initialize stopzurile, route_id, buses = nil, csv = nil
    if csv.nil? or csv == false
      incoming_obj = JSON.parse File.read(stopzurile)
    else
      incoming_obj = csv_to_obj stopzurile, route_id
    end
    @obj = ZurStops.filter_stops incoming_obj, route_id
    build_stops_by_code
  end

  def csv_to_obj filename, route_code
    puts 'begin: ZurStops.csv_to_obj'
    out_obj = {
      "features" => []
    }
  ...
  def self.filter_stops incoming_obj, route_id
    puts "begin: ZurStops.filter_stops"
    new_obj = []
    ...
    puts "end: ZurStops.filter_stops"
    new_obj
  end
  ...
  def add_passenger_loads buses
    puts "begin: ZurStops.add_passenger_loads"
    ...
    puts "end: ZurStops.add_passenger_loads"
  end
  ...
  def sort_passenger_loads
    puts "begin: ZurStops.sort_passenger_loads"
    @obj.each{|feature| 
    ...
    puts "end: ZurStops.sort_passenger_loads"
  end
