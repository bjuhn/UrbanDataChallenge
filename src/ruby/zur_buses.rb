class ZurBuses
  ...
  def load_bus_info line_stops
    puts "begin: ZurBuses.load_bus_info"
    ...
    puts "end: ZurBuses.load_bus_info"
  end
  ...
  def get_stop_lat_lngs stops
    puts "begin: ZurBuses.get_stop_lat_lngs"
    @obj.each{ |bus| 
      bus["runs"].each{|run|
    ...
    puts "end: ZurBuses.get_stop_lat_lngs"
  end

  def get_segment_ids route
    puts "begin: ZurBuses.get_segment_ids"
    @obj.each{ |bus| 
      bus["runs"].each{|run|
        i = 0
        route.get_features.each{ |feature|
          ...
          if run["tripDirection"] == "1" and (first[0].to_f - coords[0].to_f).abs < delta and (first[1].to_f - coords[1].to_f).abs < delta
            ...
            run["segment"] = i
            break
          elsif run["tripDirection"] == "2" and (last[0].to_f - coords[0].to_f).abs < delta and (last[1].to_f - coords[1].to_f).abs < delta
            puts 'found'
            run["segment"] = i
            break
          end
    ...
    puts "end: ZurBuses.get_segment_ids"  
  end

  def fill_arrivals
    puts "begin: ZurBuses.fill_arrivals"
    @obj.each{ |bus| 
      arrive_time = nil
    ...
    puts "end: ZurBuses.fill_arrivals"
  end
